<template>
	<div id="cateMain">
		<div class="has-categories-list" v-if="listProducts != ''">
					...
					<div class="row">
						<div class="col-md-12">
							<ul class="breadcrumb">
								...
								<li style="width: 50%;"><a href="javascript:void(0)"><span>SẢN PHẨM CỦA BLOOD SHOPPING MALL</span></a></li>
							</ul>
						</div>
					</div>
				</div>
			...
			<div class="container">
				<div class="row">
					<div class="title col-md-12"> <h2>SẢN PHẨM CỦA BLOOD SHOPPING MALL</h2></div>
					...
					<template v-else>
						<div class="content-had" v-if="listProducts != ''" style="width: 100%;">
							<div v-for="product in listProducts" v-bind:key="product.id_number" class="item col-6 col-md-6 col-xs-6 col-sm-4 col-lg-3 col-xl-3">
								<div class="item-product" @click="seenProductDetail(product)">
									<div class="image-product">
										<nuxt-link :to="'/product/'+ product.link"><a href="#"><img v-bind:src="$store.state.apiApp + 'products/' + product.image_main" alt="" /></a></nuxt-link>	 
									</div>
	...
	export default {
		...
		methods:{
			...
			fetchDataProductByCate(){
				axios.get(`${this.$store.state.apiLink}/products/list/show-index/${this.$route.params.id}`)
				.then(data => {
					if (data.data.successes) {
						this.listProducts = data.data.data
						this.pages = Math.ceil(data.data.count/20);
					}
				})
			...
			clickCallback(page) { this.$router.replace(`/products/${page}`) },
		},
	}
</script>
<style>
