...
<script>
    export default {
        props:{
            ...
            bigUrl:{ type:String, default:null },
            
            scroll:{ type:Boolean, default:false },

            showEidt:{ type:Boolean, default:false }
        },
        data () {
            return {
                id: null,
        ...
        methods: {
            initTime(){
                ...
                if(this.bigImgUrl){ imgsrc=this.bigImgUrl }
                else { imgsrc=this.imgUrl }
            ...
            mousemove(e){
                ...
                if(pos.x > range.maxX){ pos.x = range.maxX }

                if(pos.x < range.minX){ pos.x = range.minX }

                if(pos.y > range.maxY){ pos.y = range.maxY }
                
                if(pos.y < range.minY){ pos.y = range.minY }
            ...
            rotate(direction){
                ...
                orginImg.onload = ()=> { this.rotateImg(orginImg,direction,this.step) }
                if(this.bigOrginUrl){
                    var bigOrginImg=new Image()
                    orginImg.crossOrigin = "Anonymous";
                    bigOrginImg.src=this.bigOrginUrl
                    bigOrginImg.onload = () => { this.rotateImg(bigOrginImg,direction,this.bigStep,true) }
                }
            },
