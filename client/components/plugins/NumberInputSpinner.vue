<template>
  <div class='vnis' >
    <button @click='decreaseNumber' :class='buttonClass'>-</button>
    
    <input readonly type='number' :value='numericValue' @keypress='validateInput' @input='inputValue' :class='inputClass' :min='min' :max='max' debounce='500' />
    
    <button @click='increaseNumber' :class='buttonClass'>+</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    whileMouseDown(callback) {
      if (this.timer === null) {
        this.timer = setInterval(() => { callback(); }, this.mouseDownSpeed);
      }
    },

    increaseNumber() { this.numericValue += 1; },

    decreaseNumber() { this.numericValue -= 1; },
    ...
    validateInput(evt) {
      if (this.integerOnly === true) { this.isInteger(evt); } 
      else { this.isNumber(evt); }
    },

    inputValue(evt) { this.numericValue = evt.target.value ? parseInt(evt.target.value) : this.min; }
  },
